...
ggplot(slice_sample(curb_plot, n=10000), aes(diameter, height_range_id, colour = curb)) +
...
trees_species2 <- trees_species %>% 
  ...
  #add proportion
print(trees_species1)

count(filter(trees_species, con_or_non == "Non-Conifer"))
count(filter(trees_species, con_or_non == "Conifer"))



ggplot(trees_species1, aes(con_count, neighbourhood_name)) +
...
vancouver_trees2 <- vancouver_trees2 %>%  #I calculated age in Milestone1 but the vector was in difftime, not numeric
  mutate(age_num = as.numeric(age)) 
range(vancouver_trees2$age_num, na.rm = TRUE) #what is the spread of the data
...
count(filter(vancouver_trees2, is.na(date_planted)))
```
