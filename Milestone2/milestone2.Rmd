...
biggest_trees <- vancouver_trees2 %>% 
  ...
  filter(diameter > 0, diameter < 100, age > 10) %>% #some trees had a zero dbh, which either means that the tree was not yet 1.35m (height of DBH) or was not measured and filtered out implausible measurements, set age >10 because I wanted trees that have had some time to establish
...
<!-------------------------- Start your work below ---------------------------->
```{r}
model1 <- lm(diameter ~ curb + height_range_id + age_num, data=vancouver_trees2) #seeing if the affect curb has on diameter is significant given height and age are in the model
summary(model1)
```
...
summary(model.1)#the p-value is greater than zero but barely.
