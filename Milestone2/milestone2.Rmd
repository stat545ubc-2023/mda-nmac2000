...
biggest_trees <- vancouver_trees2 %>% 
  ...
  filter(diameter > 0, diameter < 80, age > 10) %>% #some trees had a zero dbh, which either means that the tree was not yet 1.35m (height of DBH) or was not measured, wanted trees that have had some time to establish
...
diam_filtered <- filter(vancouver_trees2, neighbourhood_name%in%c("HASTINGS-SUNRISE","ARBUTUS-RIDGE", "WEST END"), age >10, diameter>0, diameter < 80) #Top three highest mean, also trying to get rid of the large number of really small trees in the data set
